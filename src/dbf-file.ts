...
async function readRecordsFromDBF(
  ...
  try {
    ...
    let substr = (start: number, len: number, enc: string) =>
          ...
          switch (field.type) {
            ...
            case "D": // Date
              value =
                buffer[offset] === 0x20
                  ? null
                  : parse8CharDate(substr(offset, 8, encoding));
              offset += 8;
              break;
            ...
            case "M": // Memo
              ...
              while (true) {
                ...
                if (dbf._version === 0x83) {
                  ...
                  value += iconv.decode(
                    memoBuf.slice(0, eos === -1 ? memoBlockSize : eos),
                    encoding
                  );
